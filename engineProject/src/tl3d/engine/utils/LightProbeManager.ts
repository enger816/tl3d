        ...
        public getData($pos: Vector3D): Array<Vector3D> {
            ...
            for (var i: number = 0; i < this._dataAry.length; i++) {
                ...
                if (this.testPoint(lightArea, $pos)) {
                    ...
                    return this.getResultData(lightArea.posItem, Util.float2int(bp.x / lightArea.betweenNum),
                        Util.float2int(bp.z / lightArea.betweenNum), Util.float2int(bp.y / lightArea.betweenNum), lightArea.betweenNum, bp);
                }
            }
            return this._defaultVec;
        }
