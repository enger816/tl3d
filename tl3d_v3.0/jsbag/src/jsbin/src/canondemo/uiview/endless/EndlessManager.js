    ...
    var EndlessManager = /** @class */ (function (_super) {
        ...
        EndlessManager.prototype.saveEndlessDataToWeb = function () {
            if (this.layerNum > this.selfBestScore) {
                ...
                if (GameData.userInfo && GameData.userInfo.avatarUrl.length) {
                    ...
                    infoUrl = "https://api.h5key.com/static/wudiqiuqiu/ui/userpic/emptyicon.jpg";
                }
                var $postStr = "";
                $postStr += "level=" + 1;
        ...
        EndlessManager.prototype.startGame = function ($bfun) {
            ...
            if (!Boolean(this.baseSceneItem)) {
                ...
                LoadManager.getInstance().load(Scene_data.fileRoot + "endlessconfig.txt", LoadManager.XML_TYPE, function ($data) {
                    var $obj = JSON.parse($data);
                    _this.endlessConfigVo = new EndlessConfigVo;
                    _this.endlessConfigVo.levelitem = $obj.levelitem;
                    _this.endlessConfigVo.addtimeitem = $obj.addtimeitem;
                    _this.endlessConfigVo.maxtime = $obj.maxtime * 1000;
                    _this.endlessConfigVo.waitrevivetime = $obj.waitrevivetime * 1000;
                    _this.endlessConfigVo.timefinishrealplaynum = $obj.timefinishrealplaynum;
                    for (var i = 0; i < _this.endlessConfigVo.levelitem.length; i++) {
                        var $vo = new endless.EndlessMapSceneVo();
                        $vo.mapuid = _this.endlessConfigVo.levelitem[i];
                        _this.baseSceneItem.push($vo);
                    }
                    _this.loadAllScene();
                }, {
                    failfun: function () {
                        _this.failWaitLoadScene();
                    }
                });
            }
            else {
        ...
        EndlessManager.prototype.needAddNewScene = function () {
            ...
            var $showEvt = new endless.EndLessEvent(endless.EndLessEvent.MUI_SHOW_ADD_SCENE_TIME);
            $showEvt.data = this.nextShowAddTime;
            ModuleEventManager.dispatchEvent($showEvt);
