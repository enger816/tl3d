    ...
    export class ModelshowMouseManager {
        ...
        public addMouseEvent(): void {
            if (tl3d.Scene_data.isPc) {
                document.addEventListener(tl3d.MouseType.MouseDown, ($evt: MouseEvent) => { this.onMouse($evt) });
                document.addEventListener(tl3d.MouseType.MouseUp, ($evt: MouseEvent) => { this.onMouse($evt) });
                document.addEventListener(tl3d.MouseType.MouseMove, ($evt: MouseEvent) => { this.onMouse($evt) });
                document.addEventListener(tl3d.MouseType.MouseWheel, ($evt: MouseWheelEvent) => { this.onMouseWheel($evt) });
            } else {
                document.addEventListener(tl3d.MouseType.TouchMove, ($evt: TouchEvent) => { this.mouseToEvent($evt) });
                document.addEventListener(tl3d.MouseType.TouchEnd, ($evt: TouchEvent) => { this.mouseToEvent($evt) });
                document.addEventListener(tl3d.MouseType.TouchStart, ($evt: TouchEvent) => { this.mouseToEvent($evt) });
            }
        ...
        private onMouse($e: MouseEvent): void {
            var evt: tl3d.InteractiveEvent;
            var point: tl3d.Vector2D = new tl3d.Vector2D();
        ...
        private makeMouseEvent(evt: tl3d.InteractiveEvent, point: tl3d.Vector2D): void {
            var temp: boolean = tl3d.UIManager.getInstance().mouseEvetData(evt, point);
        ...
        private clikSceneGround($pos: tl3d.Vector2D): void {
