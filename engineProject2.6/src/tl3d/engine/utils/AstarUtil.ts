...
import { Groundposition } from "../math/Groundposition";
...
export class AstarUtil {
    ...
    */
    ...
    public static getBaseHeightByBitmapdata($xpos: number, $ypos: number): number {
        var perX: number = $xpos - Util.float2int($xpos);
        var perY: number = $ypos - Util.float2int($ypos);

        var zero_zero: number = this.getBitmapDataHight(Util.float2int($xpos), Util.float2int($ypos))
        var zero_one: number = this.getBitmapDataHight(Util.float2int($xpos), Math.ceil($ypos))
        var one_zero: number = this.getBitmapDataHight(Math.ceil($xpos), Util.float2int($ypos))
