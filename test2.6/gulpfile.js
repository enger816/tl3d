...
gulp.task('clean', function () {
    ...
    //  /**  当前目录所有
    //  /*.* 所有文件
    return gulp.src(['release/web/*','!release/web/version.json'], {read: false,force:true})
        .pipe(clean());
});
...
gulp.task('genmanifest', function () {
   return gulp.src('release/web/**/*.*')
        // .pipe(sass())
        .pipe(rev()) // Optional
        .pipe(assetManifest({log:false,includeRelativePath:true,manifestFile:"release/web/version.json"}))
