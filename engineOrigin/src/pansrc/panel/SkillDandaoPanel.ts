...
class SkillDandaoPanel extends Laya.Sprite {
    constructor() {
        ...
        this.uiLayaSceneChar = this.addModelChar(String(2019));
        this.charaa = this.addModelChar(String(3006));
    ...
    private addOther(): void {
        ...
        this.attactRole.setRoleUrl(getRoleUrl("3014"));
        this.attactRole.set2dPos(400, 400)
    }
    private loadSkill(): void {
        this.layaSceneLevel.scene.skillManager.preLoadSkill(getSkillUrl("300060"));
        this.layaSceneLevel.scene.skillManager.preLoadSkill(getSkillUrl("200190"));
        
    }
    private charaa: layapan.LayaSceneChar
    ...
    this.playDindianSkill(this.charaa);
    ...
    private playDindianSkill($char: layapan.LayaSceneChar): void {
        ...
        var $skill: layapan.OverrideSkill = this.layaSceneLevel.scene.skillManager.getSkill(getSkillUrl("300060"), "skill_01");
    ...
    public charPlayDanDaoSkill($char: layapan.LayaSceneChar): void {
        ...
        $char.watch(this.attactRole)
        var $skill: layapan.OverrideSkill = this.layaSceneLevel.scene.skillManager.getSkill(getSkillUrl("200190"), "skill_01");
        if (!$skill.keyAry) {
            return;
        }
        if ($skill) {
    ...
    private addModelChar($mode): layapan.LayaSceneChar {
        var $baseChar: layapan.LayaSceneChar = new layapan.LayaSceneChar();
        this.layaSceneLevel.scene.addMovieDisplay($baseChar);
        $baseChar.setRoleUrl(getRoleUrl($mode));
