...
import { Disp2DBaseText } from "../ui/compenent/Dis2DUIContianerPanel"
import { UIRectangle } from "../ui/base/UIRectangle"
import { UIManager } from "../ui/UIManager"
import { ArtFont } from "./ArtFont"
import { UiDraw } from "../ui/base/UIData"
import { Rectangle } from "../math/Rectangle"
import { UIData } from "../ui/base/UIData"
import { TextureManager } from "../material/TextureManager"
import { TimeUtil } from "./TimeUtil"
import { AlphaUICompenent } from "../ui/compenent/AlphaUiContianer"
import { Scene_data } from "../context/Scene_data"
import { TextAlign } from "../ui/base/TextAlign"
import { LabelTextFont } from "./LabelTextFont"
import { LoadManager } from "./LoadManager"
import { UICompenent } from "../ui/base/UICompenent"
import { ObjData } from "../base/ObjData"
import { UIRenderComponent } from "../ui/base/UIRenderComponent"
import { ProgrmaManager } from "../program/ProgramManager"
import { UIConatiner } from "../ui/base/UIContainer"
import { UIAtlas } from "../ui/base/UIAtlas"
import { TextureRes } from "../material/TextureRes"
import { Dis2DUIContianerPanel } from "../ui/compenent/Dis2DUIContianerPanel"
import { AlphaUiContianer } from "../ui/compenent/AlphaUiContianer"
import { Dis2DUIContianerBase } from "../ui/base/UIContainer"
...
export class TextJumpUiDrawAndRefreash extends Disp2DBaseText {
    ...
    public makeData(): void {
        if (this._data) {
            ...
            switch (vo.type) {
                ...
                case TextJumpType.N_BEATBACK:
                case TextJumpType.N_RESISTANCE:
                    //文字类
                    this.drawTxtBytext(vo, UIData.publicsUi);
                    break;
                case TextJumpType.N_NORMALDAMAGE:
                    //数字类
                    ArtFont.getInstance().writeFontToSkinName(this.parent.uiAtlas, this.textureStr, String(vo.str), "NUM" + (vo.type + 60), TextAlign.RIGHT);
                    break;
    ...
    private drawTxtBytext($vo: TextJumpUiVo, $key: string = UIData.publicUi) {
        var rec: UIRectangle = this.parent.uiAtlas.getRec(this.textureStr);
        var ctx = UIManager.getInstance().getContext2D(rec.pixelWitdh, rec.pixelHeight, false);
        ...
        if ($vo.type == TextJumpType.N_RESISTANCE || $vo.type == TextJumpType.N_IMMUNE || $vo.type == TextJumpType.N_RESURGENCE || $vo.type == TextJumpType.N_BEATBACK) {
            $length = 100;
        } else if ($vo.type == TextJumpType.N_ONCEATTACK) {
            $length = 186;
        }

        let sheight: number = $vo.type >= TextJumpType.N_NORMALDAMAGE ? 47 : 25
    ...
    protected drawTxtBydigitalAndtext($vo: TextJumpUiVo): number {
        ...
        if ($vo.type == TextJumpType.N_UPHP) {
            txtcolor = ArtFont.num83
            $width = 96
            $height = 47
        } else if ($vo.type == TextJumpType.N_CRIT) {
            txtcolor = ArtFont.num77
            $width = 91
            $height = 47
        } else if ($vo.type == TextJumpType.N_PASSIVE) {
            txtcolor = ArtFont.num81
            $width = 93
            $height = 47
        } else if ($vo.type == TextJumpType.N_RESISTANCE) {
            txtcolor = ArtFont.num83
            $width = 94
            $height = 47
        }
    ...
    public update(): void {
        ...
        if (this.nextTime < TimeUtil.getTimer() || this.renderData2.length != this._uiList.length * 4) {
            ...
            for (var i: number = 0; i < this._uiList.length; i++) {
                ...
                var b: number = ($bloodUICompenent.colortype + 1) * 16 / 64;
...
export class CharTitleMeshVo extends baseMeshVo {
    private _num: number;
...
export class CharNameMeshVo extends baseMeshVo {
...
export class BloodLineMeshVo extends baseMeshVo {
    public num: number;
...
export class JumpTextMeshVo extends baseMeshVo {
    public str: string
    public destory(): void {
        this.pos = null;
        this.clear = true
...
export class BloodManager {
    ...
    private _bloodLineUIConatiner: BloodLineUIConatiner//血条;


    public uiContianerItem: Array<Dis2DUIContianerBase>;
    public constructor() {
        this.uiContianerItem = new Array()
        this._charTitleContianerPanel = new Dis2DUIContianerPanel(CharTitleUiVo, new Rectangle(0, 0, 131, 69), 10);
        this._charNameContianerPanel = new Dis2DUIContianerPanel(CharNameUiVo, new Rectangle(0, 0, 256, 24), 50);
        this._jumpTxtContianerPanel = new AlphaUiContianer(TextJumpUiDrawAndRefreash, new Rectangle(0, 0, 299, 50), 10);
        this._expjumpTxtContianerPanel = new AlphaUiContianer(ExpTextJumpUiDrawAndRefreash, new Rectangle(0, 0, 512, 100), 5);
        this._bloodLineUIConatiner = new BloodLineUIConatiner();
    ...
    public setJumpNum($textJumpUiVo: TextJumpUiVo): void {
        // if (!$color) {
    ...
    public setExpJumpNum($textJumpUiVo: TextJumpUiVo): void {
