...
module.exports = function(options) {
    ...
    return map(function(file, callback) {
        ...
        //需改写入格式
        var tempstr=filename.substring(4,filename.length);
        tempstr=tempstr.replace(/\\/g,"/");
        var strindex=tempstr.lastIndexOf('-');
        var prestr=tempstr.substring(0,strindex);
        var endstr=tempstr.substring(strindex+1,tempstr.length);
        var arr=endstr.split('.');
        fileList[prestr+'.'+arr[1]]=arr[0];
