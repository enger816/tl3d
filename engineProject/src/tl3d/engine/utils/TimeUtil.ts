namespace tl3d {
    export class TimeUtil {
        ...
        static funAry: Array<Function> = new Array;
        static timefunAry: Array<TimeFunTick> = new Array;
        static outTimeFunAry: Array<TimeFunOut> = new Array;
        static time: number = 0;

        static getTimer(startTime:number = TimeUtil.START_TIME): number {
        ...
        **/
        ...
        static dayTime: number = 86400;
        static HourTime: number = 3600;
        static MinuteTime: number = 60;
        /**
         * 时间差转换
         * DD天HH时MM分SS秒
         */
        static getDiffTime1(nS: number): string {
            ...
            var day: number = Util.float2int(nS / this.dayTime);
            nS -= day * this.dayTime;
            var hour: number = Util.float2int(nS / this.HourTime);
            nS -= hour * this.HourTime;
            var minus: number = Util.float2int(nS / this.MinuteTime);
        ...
        **/
        ...
        static removeAllTickOut(){
            TimeUtil.outTimeFunAry = [];
        }


        static addTimeTick($time: number, $fun: Function, $beginTime: number = 0): void {
