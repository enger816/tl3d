...
export class Util {
    ...
    static hexToArgb(expColor: number, is32: boolean = true, color: Vector3D = null): Vector3D {
        if (!color) {
            color = new Vector3D();
        }
    ...
    static hexToArgbNum(expColor: number, is32: boolean = true, color: Vector3D = null): Vector3D {
    ...
    static strokeFilter(ctx: CanvasRenderingContext2D, width: number, height: number, color: number): void {
        var colorVec: Vector3D = Util.hexToArgb(color);
