...
class RoleSkill extends Laya.Sprite {
    ...
    //敌对角色
    private _animysPosition: Array<Pan3d.Vector3D> = [
        new Pan3d.Vector3D(11, 0, -64),//左上
        new Pan3d.Vector3D(48, 0, 208),//右上
        new Pan3d.Vector3D(-124, 0, 109),//第二排左一
        new Pan3d.Vector3D(-20, 0, 129),//第二排左中
        new Pan3d.Vector3D(92, 0, 153),//第二排右一

        new Pan3d.Vector3D(57, 0, -136),//右下
        new Pan3d.Vector3D(-42, 0, -154),//左下
        new Pan3d.Vector3D(120, 0, -33),//第二排右一
        new Pan3d.Vector3D(-73, 0, 209),//第二排右中
        new Pan3d.Vector3D(-102, 0, -84)//第二排右一
    ];

    //Boss站位
    private _bossPosition: Array<Pan3d.Vector3D> = [
        new Pan3d.Vector3D(-30, 0, 138),//第二排右一
        new Pan3d.Vector3D(66, 0, -320),//右下
        new Pan3d.Vector3D(-66, 0, -321),//左下
        new Pan3d.Vector3D(0, 0, -235),//第二排右一

        new Pan3d.Vector3D(119, 0, -219),//第二排右中
        new Pan3d.Vector3D(81, 0, 14),//第二排左一
        new Pan3d.Vector3D(-113, 0, -246),//第二排右一
        new Pan3d.Vector3D(-130, 0, -9),//左上
        new Pan3d.Vector3D(-73, 0, 209),//右上

        new Pan3d.Vector3D(-73, 0, 209),//第二排左中
    ...
    public initCam(): void {
        ...
        Pan3d.Scene_data.fogData = [825, 0.003];
    }
    ...
    private loadsss() {
        ...
        for (var i = 0; i < aa.length; i++) {
            ...
            // this.layaSceneLevel.scene.playLyf(getEffectUrl(element.beizhu), new Pan3d.Vector3D(180, 0, -420));
            // Pan3d.MeshDataManager.getInstance().getMeshData(getRoleUrl(element.beizhu), ()=>{});
        }
    }
    ...
    private onStartDrag(e: Event): void {
        // this.fight();
        // this._mainChar.play(Pan3d.CharAction.JUMP, 2, false);
        // Pan3d.ShockUtil.getInstance().shock(1000, 20);
    ...
    private fight() {
        ...
        if ($att_type == 1) {
            ...
            this._mainChar.play(Pan3d.CharAction.WALK, 2, false);
            //法线延长线坐标
            var tpos: Pan3d.Vector3D = new Pan3d.Vector3D(this._enemyChar[0].px, this._enemyChar[0].py, this._enemyChar[0].pz);
            var cpos: Pan3d.Vector3D = new Pan3d.Vector3D(this._mainChar.px, this._mainChar.py, this._mainChar.pz);
            var dis: number = Pan3d.Vector3D.distance(tpos, cpos);
            var normalV3: Pan3d.Vector3D = tpos.subtract(cpos);
            normalV3.normalize();
            normalV3.scaleBy(dis * 0.8);
            var $c3d: Pan3d.Vector3D = cpos.add(normalV3);
    ...
    //使用技能
    private charPlaySkill($charVo: Game3dChar, $att_type, skillfile, effectfile, $targetAry: Array<Pan3d.Vector3D> = null): void {
        //技能播放结束回调
        var comSkillFun: Function = () => {
            setTimeout(() => {
                if ($att_type == 1) {
                    $charVo.rotationY += 180;
                    $charVo.play(Pan3d.CharAction.WALK, 2, false);
                    let postion = this._isBoss ? this._bossPosition : this._animysPosition;
    ...
    private _pos: Pan3d.Vector3D = new Pan3d.Vector3D(10, 10, 10);
    private addModelChar(): Game3dChar {
        ...
        // $baseChar.play(Pan3d.CharAction.JUMP, 2, false);
