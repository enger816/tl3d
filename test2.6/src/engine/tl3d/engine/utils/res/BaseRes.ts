...
import { ObjDataManager } from "../ObjDataManager";
...
export class BaseRes extends ResCount {
  ...
  public static readIntForTwoByteNew(byte: Pan3dByteArray): Uint16Array {
    var iLen: number = byte.readInt();
    var indices: Uint16Array = new Uint16Array(iLen);
    for (var i: number = 0; i < iLen; i++) {
   ...
   * @param byte 
   */
  public static readIndexForInt(byte: Pan3dByteArray): Uint16Array {
    var iLen: number = byte.readInt();
    var indices: Uint16Array = new Uint16Array(iLen);
    for (var i: number = 0; i < iLen; i++) {
